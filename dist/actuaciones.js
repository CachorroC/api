...
async function fetcher(idProceso) {
    ...
    try {
        ...
        return actuaciones.map((actuacion) => {
            const { fechaActuacion, fechaRegistro, fechaFinal, fechaInicial, consActuacion, cant, idRegActuacion, } = actuacion;
            return {
                ...actuacion,
...
async function* AsyncGenerateActuaciones(procesos) {
    for (const { idProceso, carpetaNumero, carpetaId, } of procesos) {
        ...
        if (fetcherIdProceso && fetcherIdProceso.length > 0) {
            const actsActualizadas = await actuacion_1.default.updateAllActuaciones(fetcherIdProceso, carpetaNumero);
            console.log(actsActualizadas);
            await actuacion_1.default.prismaUpdaterActuaciones(fetcherIdProceso, carpetaNumero, carpetaId);
        }
