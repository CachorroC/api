...
for (const carpeta of carpetas_1.RawCarpetas) {
    const juzgadoByCarpeta = extrapolateIdCiudadyTipo(String(carpeta.JUZGADO_CIUDAD), carpeta.JUZGADO_EJECUCION, carpeta.JUZGADO_ORIGEN);
...
fs.writeFile('outgoingJuzgados.json', JSON.stringify(outgoingJuzgados, null, 2));
function extrapolateIdCiudadyTipo(ciudad, ejecucion, origen) {
    ...
    if (matchedRegexNumberAndLetters) {
        ...
        if (asAnArray.length === 0) {
            ...
            return {
                ...
                value: `juzgado ${asAnArray[1].padStart(3, '000')} ${temporaryTipo} de ${ciudad}`.toUpperCase(),
            };
        }
        const temporaryTipo = (0, juzgado_1.extrapolateTipoToCorrectType)(asAnArray[3]);
        return {
