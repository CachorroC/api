...
class JuzgadoClass {
    constructor({ id, tipo, ciudad, }) {
        ...
        const [matchedDespacho] = despachos_1.Despachos.filter((despacho) => {
            ...
            if (indexOfDespacho !== -1) {
                console.log(`Juzgado Class indexOf Despacho ${includesDespacho}: ${normalizedIteratedName} === ${normalizedName}: ${normalizedIteratedName === normalizedName}`);
            }
            if (includesDespacho) {
                console.log(`Juzgado Class includes Despacho${includesDespacho}: ${normalizedIteratedName} === ${normalizedName}: ${normalizedIteratedName === normalizedName}`);
    ...
    static fromLongName(despacho) {
        const matchedDespachoParts = despacho.match(/JUZGADO (\d+) ([A-Z\sñúóéíá]+) DE ([.A-Z\sñúóéíá-]+)/im);
        if (!matchedDespachoParts) {
            return new JuzgadoClass({
                id: "",
                tipo: despacho,
