...
export class ClassCarpeta {
    ...
    if ( procesosRes && Array.isArray( procesosRes.procesos ) ) {
      ...
      for ( const proceso of this.procesos ) {
        ...
        if ( actRes && Array.isArray( actRes.actuaciones ) ) {
          ...
          const ultimaActuacion = actRes.actuaciones.find( ( act ) => {
  ...
  public async persistToDb() {
    ...
    txOps.push( prisma.carpeta.upsert( {
      where: {
        numero: this.numero,
      },
      create: {
    ...
    for ( const proc of this.procesos ) {
      ...
      txOps.push( prisma.juzgado.upsert( {
        where: {
          id_tipo_ciudad: {
      ...
      txOps.push( prisma.proceso.upsert( {
        where: {
        ...
        create: {
          ...
          idConexion  : proc.idConexion || Math.floor( Math.random() * 9999 ),
    ...
    const validProcessIds = this.procesos.map( ( p ) => {
      return p.idProceso;
    } );
    ...
    for ( const act of this.actuaciones ) {
      // Find parent process
      const parentProc = this.procesos.find( ( p ) => {
      ...
      txOps.push( prisma.actuacion.upsert( {
        where: {
    ...
    if ( latestActId && latestActDate ) {
      txOps.push( prisma.carpeta.update( {
        where: {
          numero: this.numero,
        },
        data: {
    ...
    try {
      ...
      console.log( `Successfully synced Carpeta ${ this.numero }` );
    } catch ( error ) {
      console.error(
...
async function main() {
  ...
  const carpetas = rawInput.map( ( raw ) => {
