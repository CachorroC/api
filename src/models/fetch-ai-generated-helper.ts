...
export function extrapolateTipoToCorrectType(tipo: string): string {
  ...
  if (hasEjecucion) {
    if (isPequenasCausas) output = "DE PEQUEÑAS CAUSAS Y COMPETENCIA MÚLTIPLE";
    else if (isPromiscuoMunicipal) output = "PROMISCUO MUNICIPAL";
...
class FileLogger {
  ...
  log(error: any, context: object) {
    ...
    fs.appendFileSync(path.join(__dirname, this.filename), entry);
  }
}
...
export class ClassCarpeta {
  ...
  constructor(rawCarpeta: RawDb) {
    ...
    if (OBSERVACIONES) {
      ...
      extras.forEach((nota: string) => {
        notasCounter++;
        this.notas.push(new NotasBuilder(nota, Number(NUMERO), notasCounter));
      });
    }
    ...
    idBuilder = isNaN(cedulaAsNumber) ? Number(NUMERO) : cedulaAsNumber;
  ...
  public async syncWithApi() {
    ...
    if (procesosRes && Array.isArray(procesosRes.procesos)) {
      ...
      this.idProcesos = this.procesos.map((p) => p.idProceso);
  ...
  public async persistToDb(client: PrismaClient) {
    ...
    for (const proc of this.procesos) {
      ...
      await client.proceso.upsert({
        ...
        create: {
          ...
          fechaProceso: proc.fechaProceso ? new Date(proc.fechaProceso) : null,
    ...
    for (const proc of this.procesos) {
      ...
      const relevantActs = this.actuaciones.filter(
        (a) => a.llaveProceso === proc.llaveProceso,
      );
      ...
      for (const act of relevantActs) {
        await client.actuacion.upsert({
          ...
          create: {
            ...
            fechaInicial: act.fechaInicial ? new Date(act.fechaInicial) : null,
            fechaFinal: act.fechaFinal ? new Date(act.fechaFinal) : null,
            cant: act.cant,
...
async function main() {
  ...
  const carpetas = RawCarpetas.map((raw) => new ClassCarpeta(raw));
  ...
  for (const carpeta of carpetas) {
    ...
    try {
      ...
      console.error(`Error saving Carpeta ${carpeta.numero}`, e);
    }
  }
}
