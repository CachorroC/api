...
export function extrapolateTipoToCorrectType(
  ...
  const isPequeñasCausas =
    /PCCM|PCYCM|Peque|causas/gim.test(tipo);
  ...
  const isCivilCircuito =
    /(CCTO|CIRCUITO|CTO|C CTO|CC)/gim.test(tipo);
  ...
  constructor({
    ...
    const [matchedDespacho] = Despachos.filter(
      (despacho) => {
        ...
        const indexOfDespacho =
          normalizedIteratedName.indexOf(normalizedName);

        const includesDespacho =
          normalizedIteratedName.includes(normalizedName);

        if (indexOfDespacho !== -1) {
          console.log(
            `Juzgado Class indexOf ${includesDespacho}: ${normalizedIteratedName} === ${normalizedName}: ${
        ...
        if (includesDespacho) {
          console.log(
            `Juzgado Class includes${includesDespacho}: ${normalizedIteratedName} === ${normalizedName}: ${
              normalizedIteratedName === normalizedName
            }`
          );
          return true;
    ...
    if (matchedDespacho) {
      ...
      if (matchedDespachoParts) {
        const [longName, newId, newTipo, newCiudad] =
  ...
  constructor(constructorString: string) {
    ...
    const [matchedDespacho] = Despachos.filter(
      (despacho) => {
        ...
        const includesBuilded =
          normalizedIteratedName.includes(
            `${Number(this.id)} ${this.tipo}`
              .toLowerCase()
              .normalize('NFD')
              .replaceAll(/\p{Diacritic}/gu, '')
              .trim()
