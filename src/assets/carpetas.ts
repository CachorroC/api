...
/**
 * Load raw carpetas data from the Excel workbook.
 * This function encapsulates the previous script logic and returns the sorted array.
 */
export function loadRawCarpetas(): RawDb[] {
  ...
  const mapperSheets = SheetNames.flatMap((sheetname) => {
    ...
    return tableSheet.map((table) => ({
      ...table,
      category: sheetname.replaceAll(' ', '') as Category,
    }));
  });

  // Preserve original side‑effects: write intermediate JSON files.
  void fs.writeFile('outputSheets.json', JSON.stringify(outputSheets));

  const rawCarpetas = [...mapperSheets].sort((a, b) => {
    const x = a.NUMERO;
    const y = b.NUMERO;
    if (x < y) return -1;
    if (x > y) return 1;
  ...
  void fs.writeFile('carpetas.json', JSON.stringify(rawCarpetas, null, 2));
  ...
  void fs.writeFile('numbers.json', JSON.stringify(outputData, null, 2));

  return rawCarpetas;
}

// Export the data directly for existing imports.
export const RawCarpetas = loadRawCarpetas();
