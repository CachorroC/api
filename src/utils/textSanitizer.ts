 ...
 * 3. Removes unprintable ASCII control characters (range \x01-\x1F, \x7F) while preserving
 *    essential formatting characters like newlines (`\n`, `\r`) and tabs (`\t`).
 * 4. Removes the Unicode replacement character (`\uFFFD`) often resulting from encoding errors.
 * 5. Normalizes Unicode strings to Canonical Composition (NFC) form to ensure consistent
 ...
 *
 * @example
 * sanitizeText(null); // returns ''
 * sanitizeText('Hello\x00World'); // returns 'HelloWorld'
