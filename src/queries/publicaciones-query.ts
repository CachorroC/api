 ...
 */
export async function queryPublicaciones(
  juzgadoName: string, headless = false
) {
  console.log(
    `Starting query for juzgado: "${ juzgadoName }"`
  );
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `Could not find a specific option for "${ juzgadoName }". Trying to click the first available option.`
      );
    ...
    console.log(
      `Found ${ count } links matching the "Estado" criteria.`
    );
    ...
    for ( let i = 0; i < count; i++ ) {
      ...
      console.log(
        `[${ i + 1 }] ${ linkData.text } -> ${ linkData.href }`
      );
    }
    ...
    console.error(
      'An error occurred during the Playwright execution:', error
    );
...
if ( process.argv[ 1 ].includes(
  ...
  queryPublicaciones(
    targetJuzgado, headless
  ) // Default to headless for CLI unless changed
    .then(
      async (
        results
      ) => {
        ...
        console.log(
          ` Successfully retrieved ${ results.length } links and saved to ${ filePath }`
        );
        process.exit(
          0
        );
